...
Atom serial number. Atom name. Alternate location indicator. Residue name. Chain identifier. Residue sequence number. Code for insertion of residues. Orthogonal coordinates for X in Angstroms. Orthogonal coordinates for Y in Angstroms. Orthogonal coordinates for Z in Angstroms. Occupancy. Temperature factor. Element symbol, right-justified. Charge on the atom.
    '''
    def __init__(self,serial=1,name="D",altloc=" ",resname="BCD",chainid=1,resseq=1,icode=" ",x=0.0,y=0.0,z=0.0,occ=1.0,tfac=0.0,element="D",charge=0.0,hetatm=False,ister=False):
        self.name = name[0:4]
        self.serial=serial
    ...
    for i,line in enumerate(pdblines):
        ...
        if ident == "ATOM  " or ident == "HETATM" :
            ...
            if ident == "HETATM" :
                ...
                hetatm=True
            molecule.append(Atom(serial,name,altloc,resname,chainid,resseq,icode,x,y,z,occ,tfac,element,charge,hetatm))
        if line[0:3] == "TER" :
...
Test if coordinate is inside VDW radius of list of atoms
    ...
    for atom in atoms:
        if not atom.ister:
            dist=(coordinate[0]-atom.x)**2+(coordinate[1]-atom.y)**2+(coordinate[2]-atom.z)**2
            dist=math.sqrt(dist)
...
def calcmolvolume(atoms,gridspacing,probesize=0.0):
    ...
    for atom in atoms:
        myindices=grid.getindicesfromcoord([atom.x,atom.y,atom.z],mins,[gridspacing]*3)
        ...
        for shell in myshells: # 0th, 1st, 2nd shell, etc
            for shellindex in shell: # points that lay on a shell
                ...
                x,y,z=grid.getcoordfromindices(netindex,mins,[gridspacing]*3)
                testedpdbfile.write(Atom(x=x,y=y,z=z).makestring())
                if not occupiedflags[netindex[0]][netindex[1]][netindex[2]]:
                    volume+=gridvolume
                    occupiedflags[netindex[0]][netindex[1]][netindex[2]]=True
                    occupiedpdbfile.write(Atom(x=x,y=y,z=z).makestring())
